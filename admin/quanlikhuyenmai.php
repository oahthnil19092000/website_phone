                ...
                <div class="grid__row app__content">
                    ...
                    <div class="grid__column-10">
                        <div class="home-filter"> 
                            <h3 class="home-filter__header">Quản lí khuyến mãi</h3>
                            <div class="btn_themsp">
                                <form method="get" action="">
                                    <div class="timkiem">
                                        <input type="text" name="search" placeholder="Tìm kiếm tên khuyến mãi">
                                ...
                                <button type="button" class="nut nut--themsp" onclick="window.location.href='../themkhuyenmai.php'">Thêm khuyến mãi</button>
                            </div>
                            <table>
                                <tr>
                                    <th style="width: 25px">STT</th>
                                    <th style="width: 70px">Mã khuyến mãi</th>
                                    <th style="width: 100px">Tên khuyến mãi</th>
                                    <th style="width: 150px">Thời gian bất đầu</th>
                                ...
                                if (isset($_GET['search'])) {
                                    ...
                                    for ($z = count($search); $z >= 1; $z--) {
                                        ...
                                        if ($z != 1) {
                                            ...
                                            $a = implode(' ', $string);
                                            unset($string);
                                            $query = "SELECT * FROM `khuyen mai` WHERE `ten khuyen mai` LIKE '%" . $a . "%'";
                                            $datasql = mysqli_query($connect, $query);
                                            while ($row = mysqli_fetch_array($datasql, 1)) {
                                                if(!empty($data)){
                                                    if (!in_data($data, $row['ten khuyen mai']))
                                                        $data[] = $row;
                                                }else {
                                                    $data[] = $row;
                                                }
                                            ...
                                            for ($i = 0; $i < count($search); $i++) {
                                                $query = "SELECT * FROM `khuyen mai` WHERE `ten khuyen mai` LIKE '%" . $search[$i] . "%'";
                                                $datasql = mysqli_query($connect, $query);
                                                while ($row = mysqli_fetch_array($datasql, 1)){
                                                    if(!empty($data)){
                                                        if (!in_data($data, $row['ten khuyen mai']))
                                ...
                                <tr>
                                    <td><?php echo $i+1?></td>
                                    <td><?php echo $data[$i]['ma khuyen mai'] ?></td>
                                    <td><?php echo $data[$i]['ten khuyen mai'] ?></td>
                                    <td><?php 
                                    $date=date_create($data[$i]['tg bat dau']);
                                    echo "từ ngày ".date_format($date,"d - m - Y");
                                    ?></td>
                                    <td><?php
                                    $date=date_create($data[$i]['tg ket thuc']);
                                    echo "hết ngày ".date_format($date,"d - m - Y");
                                    ?></td>
                                    <td>
                                        <a href="../themkhuyenmai.php?ma=<?php echo $data[$i]['ma khuyen mai'] ?>&ten=<?php echo $data[$i]['ten khuyen mai'] ?>&bd=<?php 
                                    $date=date_create($data[$i]['tg bat dau']);
                                    ...
                                    ?>" class="icon-sua">
                                        ...
                                        <a href="../server.php?xoakhuyenmainay=<?php echo $data[$i]['ma khuyen mai'] ?>" class="icon-xoa">
