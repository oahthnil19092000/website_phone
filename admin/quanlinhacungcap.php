                ...
                <div class="grid__row app__content">
                    ...
                    <div class="grid__column-10">
                        <div class="home-filter"> 
                            <h3 class="home-filter__header">Quản lí nhà cung cấp</h3>
                            <div class="btn_themsp">
                                <form method="get" action="">
                                    <div class="timkiem">
                                        <input type="text" name="search" placeholder="Tìm kiếm tên nhà cung cấp">
                                        <button type="submit" class="icon_timkiem">
                                            <i class=" fas fa-search"></i>
                                        </button>
                                    </div>
                                </form>
                                <button class="nut nut--themsp" onclick="window.location.href='../themnhacungcap.php'">Thêm nhà cung cấp</button>
                            </div>
                            <table>
                                <tr>
                                    <th style="width: 5%">STT</th>
                                    <th style="width: 12%">Mã nhà cung cấp</th>
                                ...
                                $query = "SELECT * FROM `nha cung cap` WHERE `ma ncc`!='0'";
                                ...
                                if (isset($_GET['search'])) {
                                    ...
                                    for ($z = count($search); $z >= 1; $z--) {
                                        ...
                                        if ($z != 1) {
                                            ...
                                            $a = implode(' ', $string);
                                            unset($string);
                                            $query = "SELECT * FROM `nha cung cap` WHERE `ten ncc` LIKE '%" . $a . "%'";
                                            $datasql = mysqli_query($connect, $query);
                                            while ($row = mysqli_fetch_array($datasql, 1)) {
                                                if(!empty($data)){
                                                    if (!in_data($data, $row['ten ncc']))
                                                        $data[] = $row;
                                                }else {
                                                    $data[] = $row;
                                                }
                                            ...
                                            for ($i = 0; $i < count($search); $i++) {
                                                $query = "SELECT * FROM `nha cung cap` WHERE `ten ncc` LIKE '%" . $search[$i] . "%'";
                                                $datasql = mysqli_query($connect, $query);
                                                while ($row = mysqli_fetch_array($datasql, 1)){
                                                    if(!empty($data)){
                                                        if (!in_data($data, $row['ten ncc']))
                                ...
                                <tr>
                                    <td><?php echo $i+1?></td>
                                    <td><?php echo $data[$i]['ma ncc'] ?></td>
                                    <td><?php echo $data[$i]['ten ncc'] ?></td>
                                    <td><?php echo $data[$i]['dia chi ncc'] ?></td>
                                    <td>
                                        <a href="../themnhacungcap.php?ma=<?php echo $data[$i]['ma ncc'] ?>&ten=<?php echo $data[$i]['ten ncc'] ?>&diachi=<?php echo $data[$i]['dia chi ncc'] ?>" class="icon-sua">
                                           <i class="sp_icon fas fa-edit">Sửa</i>
                                        </a>
                                        <a href="../server.php?xoanhacungcapnay=<?php echo $data[$i]['ma ncc'] ?>" class="icon-xoa">
