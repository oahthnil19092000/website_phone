                        ...
                        <div class="home-sanpham">
                            <table>
                                ...
                                for($i=0;$i<count($data);$i++){
                                    $connect = new mysqli("localhost", "root", "", "mobile");
                                    mysqli_set_charset($connect, "utf8");
                                    $query = "SELECT b.*, a.* FROM `san pham cu the` a JOIN `phieu bao hanh` b ON (a.`so seri`=b.`so seri`) WHERE b.`ngay bao hanh`='0000-00-00' AND `ma sp`='".$data[$i]['ma sp']."'";
                                    unset($data8);
                                    $datasql = mysqli_query($connect, $query);
                                    while ($row = mysqli_fetch_array($datasql, 1)) {
                                        $data8[] = $row;
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <form method="get" action="server.php" class="soluong abc">
                                            <button type="button" onclick="window.location.href='server.php?giam=<?php echo $data[$i]['ma sp']?>&madhduyet=<?php echo $data[$i]['ma gh']?>'">-</button>
                                            <input type="text" name="thaydoi" value="<?php echo $data[$i]['ma sp']?>" hidden>
                                            <input type="text" name="madhduyet" value="<?php echo $data[$i]['ma gh']?>" hidden>
                                            <input type="number" name="soluong" id="soluong" value="<?php echo $data[$i]['so luong'] ?>" min="1" required max="<?php echo $tongsosp ?>" onblur="document.getElementsByClassName('abc')[<?php echo $i?>].submit();">
                                            <button type="button" id="add" onclick="window.location.href='server.php?tang=<?php echo $data[$i]['ma sp']?>&madhduyet=<?php echo $data[$i]['ma gh']?>'">+</button>
                                        </form>
                                    </td>
                                    <td><?php
                                    ...
                                    <td>
                                        <?php 
                                        if($max!=0){
                                            echo "<del>".number_format($data[$i]['gia sp']*$data[$i]['so luong'], 0, ',', '.')."đ"."</del><br>";
                                        }
                                        echo number_format($data[$i]['gia sp']*$data[$i]['so luong']-$data[$i]['gia sp']*$data[$i]['so luong']*$max/100, 0, ',', '.')."đ"?>
                                    </td>
                                    <td>
                                        <a href="server.php?xoaspgiohang=<?php echo $data[$i]['ma sp']?>"  class="iconhd">
                                            <div class="nuthdh">Xóa</div>
                                        </a>
                                    </td>
                                    <?php if($i==0){?>
                                    <td rowspan="<?php echo count($data); ?>" >
                                    <div style="display:flex"> 
                                        <a href="thanhtoan/?thanhtoan=<?php echo $data[$i]['so hd']?>"  class="iconhd">
